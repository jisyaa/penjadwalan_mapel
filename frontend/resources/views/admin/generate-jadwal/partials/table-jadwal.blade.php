@php
    ...
    if (isset($jadwal) && !empty($jadwal)) {
        ...
        if ($kelas_list->isEmpty()) $kelas_list = collect(['IX A', 'IX B', 'IX C']);
        ...
        $bentrok = [];
        foreach ($jadwal as $j) {
            if (!isset($j['hari'], $j['guru']) || empty($j['guru'])) continue;
                    ...
                    @foreach ($jam_list as $index => $jamKey)
                        @php
                            ...
                            if ($isKeterangan && $keteranganData) {
                                ...
                                $firstJadwal = isset($jadwal_per_hari[$hari][$jamKey]) ? reset($jadwal_per_hari[$hari][$jamKey]) : null;
                                if ($firstJadwal && isset($firstJadwal['jam']) && !is_null($firstJadwal['jam']) && $firstJadwal['jam'] !== '') {
                            ...
                            @else
                                @foreach ($kelas_list as $kelas)
                                    @php
                                        ...
                                        $isBentrok = isset($currentData) && isset($bentrok[$currentData['hari'] . '-' . $jamKey . '-' . $currentData['guru']]) && $bentrok[$currentData['hari'] . '-' . $jamKey . '-' . $currentData['guru']] > 1;
                                    @endphp
                                    <td class="text-center editable-cell"
                                        ...
                                        <select class="dropdown-select" style="display: none; width: 100%;" onchange="updateCell(this)">
